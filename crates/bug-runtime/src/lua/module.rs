...
pub(crate) struct HostInner {
    ...
    pub(crate) fsm_path: PathBuf,
    pub(crate) fsm_source: Vec<u8>,
...
impl LuaHost {
    ...
    pub fn from_fsm_source(
        ...
        let fsm = sandbox
            ...
            .map_err(|error| {
                ScriptError::from_mlua(error, "validating FSM source")
                    .with_path(source_name.as_ref())
            })?;
        validate_module(&fsm, "FSM", &["api_version", "create"])
            .map_err(|error| error.with_path(source_name.as_ref()))?;
        require_function(&fsm, "create").map_err(|error| {
            ScriptError::from_mlua(error, "validating FSM source").with_path(source_name.as_ref())
        })?;
        let fsm_proxy = readonly_proxy(&lua, fsm, "validated FSM module").map_err(|error| {
            ScriptError::from_mlua(error, "protecting validated FSM module")
                .with_path(source_name.as_ref())
        })?;
        drop(fsm_proxy);
        lua.gc_collect()
            .and_then(|()| lua.gc_collect())
            .map_err(|error| {
                ScriptError::from_mlua(error, "collecting validated FSM module")
                    .with_path(source_name.as_ref())
            })?;
        ...
        Ok(Self {
            inner: Rc::new(HostInner {
                ...
                sandbox,
                options,
                fsm_path: source_name.as_ref().to_path_buf(),
                fsm_source: source.to_vec(),
    ...
    pub fn load_behavior_descriptor(
            ...
            .and_then(Weak::upgrade)
        {
            if existing.descriptor != descriptor {
                return Err(ScriptError::contract(
                    ...
                    format!(
                        "species '{}' was already loaded with a different executable descriptor \
                         from {}",
                        descriptor.species_id,
                        existing.descriptor.behavior_path.display()
                    ),
                )
        ...
        let validated = evaluate_behavior_module(&self.inner, &descriptor)?;
        drop(validated);
        self.inner
            .lua
            .gc_collect()
            ...
            .map_err(|error| {
                ScriptError::from_mlua(error, "collecting validated Lua behavior module")
                    .with_species(&descriptor.species_id)
                    .with_path(&descriptor.behavior_path)
            })?;

        let species_id = descriptor.species_id.clone();
        ...
        self.inner
            .modules
            .borrow_mut()
            .insert(species_id, Rc::downgrade(&inner));
...
pub(crate) struct BehaviorInner {
    ...
    pub(crate) descriptor: BehaviorDescriptor,
}
...
impl Debug for BehaviorModule {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        formatter
            ...
            .field("species_id", &self.inner.descriptor.species_id)
            .field("behavior_path", &self.inner.descriptor.behavior_path)
            .field("part_count", &self.inner.descriptor.part_indices.len())
            .finish()
    }
}
...
impl BehaviorModule {
    ...
    pub fn species_id(&self) -> &str {
        &self.inner.descriptor.species_id
    }

    #[must_use]
    pub fn behavior_path(&self) -> &Path {
        &self.inner.descriptor.behavior_path
    }

    #[must_use]
    pub fn part_count(&self) -> usize {
        self.inner.descriptor.part_indices.len()
    }
}

pub(crate) fn evaluate_behavior_module(
    host: &HostInner,
    descriptor: &BehaviorDescriptor,
    ...
    let module = host
        ...
        .load_module(
            ...
            &descriptor.source,
            &source_name,
            host.options.instruction_limit,
        )
        .map_err(|error| {
    ...
    readonly_proxy(&host.lua, module, "behavior module").map_err(|error| {
...
pub(crate) fn evaluate_fsm_module(host: &HostInner) -> Result<Table, ScriptError> {
    let source_name = host.fsm_path.to_string_lossy();
    let module = host
        .sandbox
        .load_module(
            &host.lua,
        ...
        .map_err(|error| {
            ScriptError::from_mlua(error, "loading controller FSM module").with_path(&host.fsm_path)
        })?;
    validate_module(&module, "FSM", &["api_version", "create"])
        .map_err(|error| error.with_path(&host.fsm_path))?;
    require_function(&module, "create").map_err(|error| {
        ScriptError::from_mlua(error, "validating controller FSM module").with_path(&host.fsm_path)
    })?;
    readonly_proxy(&host.lua, module, "controller FSM module").map_err(|error| {
        ScriptError::from_mlua(error, "protecting controller FSM module").with_path(&host.fsm_path)
    })
}
...
fn validate_options(options: &LuaHostOptions) -> Result<(), ScriptError> {
        ...
        || options.maximum_lua_file_bytes == 0
    {
        return Err(ScriptError::new(
            ...
            "all limits must be positive and instructions must be at least 100",
        ));
    }
    Ok(())
}
